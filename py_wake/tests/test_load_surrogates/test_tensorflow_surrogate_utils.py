...
def test_TensorflowSurrogate():
    surrogate = TensorflowSurrogate.from_h5_json(example_data_path +
                                                 "iea34_130rwt/one_turbine/electrical_power", 'operating')
...
def test_bounds_warning():
    ...
    assert surrogate.predict_output(np.array([25., .1, .1])[na], bounds='ignore') == 3388877.99257053
